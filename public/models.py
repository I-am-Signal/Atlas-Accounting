...
class Credential(db.Model, BaseColumnMixin):
    ...
    expirationDate = db.Column(
        db.DateTime, default=lambda: datetime.now() + timedelta(days=365)
    )
...
class Company(db.Model, BaseColumnMixin):
    ...
    db.relationship(
        "User", backref="companies_created", foreign_keys=[creator_of_company]
    )
...
class Account(db.Model, BaseColumnMixin, CreatedByMixin):
    ...
    db.relationship(
        "User", backref="account_created_by", foreign_keys=[CreatedByMixin.created_by]
    )
...
class Transaction(db.Model, BaseColumnMixin, CreatedByMixin):
    ...
    side_for_transaction = db.Column(
        db.String(150), nullable=False
    )  # e.g., 'debit' or 'credit'
    account_number = db.Column(
        db.Integer, db.ForeignKey("account.number"), nullable=False
    )
