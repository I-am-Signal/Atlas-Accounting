...
def login_required_with_password_expiration(f):
    ...
    def login_with_expiration(*args, **kwargs):
        ...
        if datetime.now() >= curr_pass.expirationDate:
            ...
            flash(
                (
                    f"Your password has expired. Please use forgot password to change your password."
                )
            )
    ...
    if request.method == "POST":
        ...
        def checkIfAccountCanLogIn(user):
            ...
            if not True == user.is_activated:  # has account been activated
                flash(
                    "Account is not currently active. Please contact your Company administrator.",
                    category="error",
                )
                return False
            ...
            if suspended:  # is this account suspended
                flash(
                    "Account is currently suspended. Please contact your Company administrator.",
                    category="error",
                )
        ...
        if checkIfAccountCanLogIn(user):
            try:
                password_entry = (
                    Credential.query.filter_by(user_id=user.id)
                    .order_by(desc(Credential.create_date))
                    .first()
                )

                if (
                    check_password_hash(password_entry.password, password)
                    and password_entry.failedAttempts < 3
                ):
                    ...
                    flash(
                        "Incorrect password was used 3 or more times. Your account is now deactivated. Please contact your Company administrator."
                    )
                    password_entry.failedAttempts += 1
...
def sign_up():
    ...
    if request.method == "POST":
        ...
        if user:
            flash(
                f"Email already exists with Company #{user.company_id}",
                category="error",
            )
            ...
            if not company_id:
                ...
                new_company = Company(creator_of_company=user.id)
                db.session.add(new_company)
                user.company_id = (
                    Company.query.filter_by(creator_of_company=user.id).first().id
                )
                login_user(new_user, remember=True)
                flash(
                    f"Account created with username {user.username}! Welcome to Atlas Accounting.",
                    category="success",
                )
            else:
                flash(
                    f"Account was created with username {user.username}. Please contact your Company administrator for account activation. Welcome to Atlas Accounting.",
                    category="success",
                )
...
def update_password():
    ...
    if request.method == "POST":
        ...
        if not check_password_hash(previous_password.password, curr_password):
            ...
            flash(password_validity, category="error")
        elif checkAllPreviousPasswords(
            newpassword, current_user.id
        ):  # check if new password == an old one
    ...
    return render_template("update_password.html", user=current_user, homeRoute="/")
